...
CREATE TABLE IF NOT EXISTS HR.countries (
  ...
  REGION_ID INT DEFAULT NULL, --changed to the same data type on regions table
  PRIMARY KEY (COUNTRY_ID),
  FOREIGN KEY COUNTR_REG_FK (REGION_ID) REFERENCES regions(REGION_ID), --added foreign key
  CONSTRAINT counregid_check CHECK (REGION_ID >= 0) --added CHECK constraint to avoid negative values
...
CREATE TABLE IF NOT EXISTS HR.departments (
  ...
  CONSTRAINT depid_nonzero CHECK (DEPARTMENT_ID > 0), --added CHECK constraint to avoid negative values
  CONSTRAINT mgrid_check CHECK (MANAGER_ID >= 0), --added CHECK constraint to avoid negative values
  CONSTRAINT locid_check CHECK (LOCATION_ID >= 0) --added CHECK constraint to avoid negative values
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
--
...
CREATE TABLE IF NOT EXISTS HR.job_history (
  ...
  FOREIGN KEY JHIST_JOB_FK (JOB_ID) REFERENCES jobs(JOB_ID), --added foreign key
  CONSTRAINT jobempid_nonzero CHECK (EMPLOYEE_ID > 0), --added CHECK constraint to avoid negative values
  CONSTRAINT jobdepid_check CHECK (DEPARTMENT_ID >= 0) --added CHECK constraint to avoid negative values
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS HR.regions (
  ...
  CONSTRAINT regid_nonzero CHECK (REGION_ID > 0) --added CHECK constraint to avoid negative values
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
